...
code(
    "# NB1: Run notebook 1 — SFT mini checkpoint (chạy thẳng .py, không cần jupytext)\n",
    ...
    "\n",
    "print('Running NB1 — SFT mini...')\n",
    "r = subprocess.run(\n",
    "    ['python', str(WORK / 'notebooks/01_sft_mini.py')],\n",
    "    capture_output=False, text=True, cwd=WORK,\n",
    "    env={**os.environ}\n",
    ")\n",
    "if r.returncode == 0:\n",
    "    print('✓ NB1 SFT-mini DONE')\n",
    "else:\n",
    "    raise RuntimeError(f'NB1 failed (exit {r.returncode}). Check output above.')\n",
)
...
code(
    ...
    "from pathlib import Path\n",
    "WORK = Path('/content/lab22')\n",
    "print('Running NB2 — Preference data...')\n",
    "r = subprocess.run(\n",
    "    ['python', str(WORK / 'notebooks/02_preference_data.py')],\n",
    "    capture_output=False, cwd=WORK, env={**os.environ}\n",
    ")\n",
    "if r.returncode == 0:\n",
    "    parquet = WORK / 'data/pref/train.parquet'\n",
    "    print(f'✓ NB2 DONE — {parquet} ({parquet.stat().st_size/1e6:.1f} MB)')\n",
    "else:\n",
    "    raise RuntimeError(f'NB2 failed (exit {r.returncode})')\n",
)
...
code(
    ...
    "from pathlib import Path\n",
    "WORK = Path('/content/lab22')\n",
    "print('Running NB3 — DPO Training (~15-20 min)...')\n",
    "r = subprocess.run(\n",
    "    ['python', str(WORK / 'notebooks/03_dpo_train.py')],\n",
    "    capture_output=False, cwd=WORK, env={**os.environ}\n",
    ")\n",
    "if r.returncode == 0:\n",
    ...
    "else:\n",
    "    raise RuntimeError(f'NB3 failed (exit {r.returncode})')\n",
)
...
code(
    ...
    "WORK = Path('/content/lab22')\n",
    "print('Running NB4 — Side-by-side eval...')\n",
    "r = subprocess.run(\n",
    "    ['python', str(WORK / 'notebooks/04_compare_and_eval.py')],\n",
    "    capture_output=False, cwd=WORK, env={**os.environ}\n",
    ")\n",
    "if r.returncode == 0:\n",
    "    judge_f = WORK / 'data/eval/judge_results.json'\n",
    "    if judge_f.exists():\n",
    ...
    "    else:\n",
    "        print('✓ NB4 DONE (judge results not yet written)')\n",
    "else:\n",
    "    raise RuntimeError(f'NB4 failed (exit {r.returncode})')\n",
)
...
code(
    ...
    "from pathlib import Path\n",
    "WORK = Path('/content/lab22')\n",
    "print('Running NB5 — Merge + GGUF export...')\n",
    "r = subprocess.run(\n",
    "    ['python', str(WORK / 'notebooks/05_merge_deploy_gguf.py')],\n",
    "    capture_output=False, cwd=WORK, env={**os.environ}\n",
    ")\n",
    ...
    "else:\n",
    "    print('⚠ NB5 may have warnings — check GGUF files above')\n",
)
...
code(
    ...
    "print('Running NB6 — Benchmark suite (~30 min)...')\n",
    "r = subprocess.run(\n",
    "    ['python', str(WORK / 'notebooks/06_benchmark.py')],\n",
    "    capture_output=False, cwd=WORK, env={**os.environ}\n",
    ")\n",
    ...
    "else:\n",
    "    print('⚠ NB6: benchmark_results.json missing (check output above)')\n",
)
